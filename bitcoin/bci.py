...
def set_network(*args):
    '''Decides if args are mainnet or testnet and returns args, network'''
    argz = args
    if not argz:
        return "btc"
    r = []
    ...
    return "testnet" if all(r) else "btc"

### TODO: fix 
#   >>> ads, nw = parse_addr_args([taddr, taddr2], "testnet")
#.  >>> ads
#.  >>> ([taddr, taddr], )
def parse_addr_args(*args):
    ...
    #    addr_args = args
    if len(argz) >= 1 and argz[-1] in ('testnet', 'btc'):
        addr_args = argz[:-1]
    network = set_network(*argz)
    #else:
    #    argz = args
    if len(argz) == 1 and isinstance(argz, list):
