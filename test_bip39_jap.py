...
class TestBIP39(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        print('Testing BIP39 Japanese')

    def test_all(self):

        BIP39_VECTORS_JAPANESE = \
        [
            {
        ...
        for v in BIP39_VECTORS_JAPANESE:
            self.assertTrue(mnemonic.bip39_check(v['mnemonic']))
            self.assertEqual(mnemonic.bip39_hex_to_mn(v['entropy']), v['mnemonic'])
            self.assertEqual(mnemonic.bip39_mn_to_hex(v['mnemonic'], v['passphrase']), v['seed'])
