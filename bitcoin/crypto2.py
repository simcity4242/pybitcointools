...
def get_byte(c):
    return ord(c) if str==bytes else c
...
def pbkdf2_single(password, salt, key_length, prf):
    '''Returns the result of the Password-Based Key Derivation Function 2 with
       ...
       prf - a psuedorandom function See http://en.wikipedia.org/wiki/PBKDF2
    '''
...
def bin_scrypt(password, salt, N=2**14, r=8, p=8, dkLen=64):
    h = scrypt_hash(password, salt, N, r, p, dkLen)
    return h

bip38_scrypt = bin_scrypt