...
if globals().get('OPCODES_BY_NAME', None) is None:
...
def get_op(s):
    ...
    if isinstance(s, int):
        ...
        return OPCODES_BY_NAME['OP_0']
    elif isinstance(s, string_types):
        return OPCODES_BY_NAME.get(
            s.upper() if not s.startswith("OP_") else str(s[2:]).upper(), 0)
    else:
        raise ValueError("Bad OP code: %s" % str(s))
